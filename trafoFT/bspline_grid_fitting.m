...
% FD: In which grid quadrant are the respective coordinates?
gx  = floor(X(:,1)/Spacing(1)); 
gy  = floor(X(:,2)/Spacing(2)); 

% Calculate b-spline coordinate within b-spline cell, range 0..1
% FD: Which relative coordinates does a coordinate have within its grid
% quadrant?
ax  = (X(:,1)-gx*Spacing(1))/Spacing(1);
ay  = (X(:,2)-gy*Spacing(2))/Spacing(2); 

% FD: Shift by 2 towards the center, since cubic spline interpolation needs a
% support of 3
gx=gx+2; 
gy=gy+2; 
...
[ix,iy]=ndgrid(-1:2,-1:2); 
ix=ix(:); 
iy=iy(:);

indexx=repmat(gx,[1 16])+repmat(ix',[size(X,1)  1]); 
indexx=indexx(:);

indexy=repmat(gy,[1 16])+repmat(iy',[size(X,1)  1]); 
indexy=indexy(:);
