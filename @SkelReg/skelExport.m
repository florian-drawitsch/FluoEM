...
if isfield(obj.skeletons,'em')
    ...
    obj.skeletons.em.skelExport(fullfile(targetPath, [tmp,'.nml']))
end

if isfield(obj.skeletons,'lm')
    [~, tmp] = fileparts(obj.paths.fpathLM);
    obj.skeletons.lm.skelExport(fullfile(targetPath, [tmp,'.nml']))
end

if isfield(obj.skeletons,'lm_at')
    [~, tmp] = fileparts(obj.paths.fpathLM);
    obj.skeletons.lm_at.skelExport(fullfile(targetPath, [tmp,'_AT.nml']))
end

if isfield(obj.skeletons,'lm_at_ft')
    [~, tmp] = fileparts(obj.paths.fpathLM);
    obj.skeletons.lm_at_ft.skelExport(fullfile(targetPath, [tmp,'_AT_FT.nml']))
end
