...
% INPUT name: cell Array, char string
%           String/strings to look for in the tree names.
...
for i=1:length(name)
    switch mode
        case 'partial'
            modeFun = @(x) contains(x, name{i});
        case 'exact'
            modeFun = @(x) strcmp(x, name{i});
        case 'insensitive'
            modeFun = @(x) strcmpi(x, name{i});
        case 'regexp'
            modeFun = @(x) ~isempty(regexp(x, name{i}, 'once'));
        case 'first'
            modeFun=  @(x) strncmp(x,name{i},size(name{i},2));
        otherwise
            error('search mode for treename is incorrect');
    end
    
    curIdx = find(cellfun(modeFun,skel.names));
    treeIdx=[treeIdx;curIdx];
end
